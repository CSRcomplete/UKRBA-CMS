...
export async function convertLeadToMember({ leadId, planName, changeReason }: ConvertLeadParams) {
  ...
  try {
    ...
    const statusName = cleanPlanName.startsWith("Subscribed -")
      ? cleanPlanName
      : `Subscribed - ${cleanPlanName}`;

    // 3. Find or Create crm_Lead_Statuses record
